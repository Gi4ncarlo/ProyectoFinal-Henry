...
const Home: React.FC = () => {
 ...
 useEffect(() => {
  const checkUserSession = async () => {
    ...
    try {
      const googleResponse = await fetch(`(${AUTHURL})/api/auth/me`, {
        method: 'GET',
      ...
      if (googleResponse.ok) {
        ...
        if (googleUser?.email && googleUser?.sid) {
          // Usuario de Google encontrado, logueamos en el backend usando el sid como password
          const loginResponse = await fetch(`${APIURL}/auth/signup`, {
            ...
            body: JSON.stringify({
              ...
              password: googleUser.sid, // Usamos el sid como contraseña
            }),
          });
