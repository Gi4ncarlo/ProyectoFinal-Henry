...
const ProviderCard: React.FC<IProviderCardProps> = ({ providerName, description, imageUrl, rating }) => {
  // Aseguramos que el rating tenga un valor numérico y asignamos 0 si es undefined o null
  const flooredRating = typeof rating === 'number' && !isNaN(rating) ? Math.floor(rating) : 0;

  return (
    <div className="w-full max-w-sm bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700">
      <Image
        className="p-8 rounded-t-lg"
        src={imageUrl || '/default-image.jpg'}  // Agregar imagen predeterminada si no hay imagen
        alt={`${providerName} image`}
        width={400}
        height={250}
      />
        ...
        <div className="flex items-center mt-2.5 mb-5">
          {Array.from({ length: 5 }).map((_, index) => (
            <svg
              ...
              fill="currentColor"
              viewBox="0 0 20 20"
              aria-label={`Rating ${index + 1} star`}  // Mejora en accesibilidad
            >
          ...
          <span className="ml-2 text-sm font-medium text-gray-500 dark:text-gray-400">{rating ? rating.toFixed(1) : 'N/A'}</span>
        </div>

        {/* Botón de contratar servicios */}
        <button
