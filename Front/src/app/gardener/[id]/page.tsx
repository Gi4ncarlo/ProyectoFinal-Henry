...
const ProviderDetail: React.FC = () => {
  ...
  // Fetch gardener data and services when component mounts
  const fetchGardenerAndServices = async () => {
    if (!id) {
      setError("El ID del proveedor no es válido.");
      return;
    }
    ...
    try {
      ...
      console.error("Error buscando información del jardinero:", err);
    ...
    try {
      ...
      console.error("Error buscando los servicios:", err);
    ...
    if (typeof window !== "undefined") {
      ...
      if (storedSession) {
        try {
          ...
          console.error("Error al recuperar la sesión del usuario:", err);
        }
      }
    }
  };

  // UseEffect to fetch data and session
  useEffect(() => {
    fetchGardenerAndServices();
    loadUserSession();
  }, [id]);
  ...
  const handleHireClick = async () => {
    ...
    if (!gardener || !selectedDate) {
      setError("Debe seleccionar una fecha y un jardinero.");
      return;
    }
    ...
    try {
      ...
      console.error("Error contratando servicios:", err);
      ...
      <div className="max-w-3xl mx-auto mt-32 mb-14 p-6 bg-white rounded-lg shadow-lg">
        ...
        <div className="mt-6">
          ...
          {gardener && (
            <GardenerCalendar
              gardenerId={gardener.id.toString()}
              onDateSelect={(date: string) => setSelectedDate(date)} // Pasar la fecha seleccionada al estado
            />
          )}
        </div>
